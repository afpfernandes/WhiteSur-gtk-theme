...
while [[ $# -gt 0 ]]; do
  ...
  case "${1}" in
    ...
    -f|--firefox|-e|--edit-firefox)
      ...
      if ! has_command librewolf && ! has_command firefox && ! has_command firefox-bin && ! has_flatpak_app org.mozilla.firefox && ! has_snap_app firefox && ! has_command firefox-developer-edition; then
